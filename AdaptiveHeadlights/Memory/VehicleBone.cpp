...
void VehicleBones::RotateAxisAbsolute(Vehicle vehicle, int index, const std::vector<SAxisRotation>& rotations) {
    ...
    NativeMatrix4x4 oo = Scaling(scalar);
    for (const auto& [axis, rotation] : rotations) {
        oo = oo * RotationAxis(axis, rotation);
    }

...
static void* CVehicleStreamRenderGfx_GetModFragType_detour(void* thisStreamRenderGfx, uint8_t modType) {
    ...
    auto script = AdaptiveHeadlights::GetScript();
    if (!script) {
        return origFragType;
    }

    if (!script->ActiveConfig()->Correction.Enable)
        return origFragType;
    ...
    {
        VehicleModRenderData renderData{ *vehicle, streamRenderGfx, modType };
        ...
        if (auto it = clonedFragTypes.find(renderData);
            ...
            LOG(DEBUG, "Inserted ({} {} {})", *vehicle, streamRenderGfx, modType);
        }
        else {
    ...
    auto adjust = deg2rad(script->ActiveConfig()->Correction.PitchAdjustMods);
    ...
    for (int i = 0; i < numBones; i++) {
        ...
        const auto& tuningBones = script->ActiveConfig()->Bones.Mods;
